...
public class GraphicEditor{
    ...
    public void insert(int draw) { // 노드 삽입
        Shape graphic;
        switch (draw) {
            case 1 -> graphic = new Line();
            case 2 -> graphic = new Rect();
            case 3 -> graphic = new Circle();
            default -> {
        ...
        if(head == null) { // head가 아무것도 가리키지 않으면
            head = graphic; // head랑 tail이 새로운 노드를가리킴
            tail = graphic;
        }
        else {
            tail.setNext(graphic); // 끝 노드에 새로운 노드를 연결하고,
            tail = graphic; // 끝 노드는 새로 만들어진 노드를 가리키게 함
        }
    }
